...
open class KotlinCocoapodsPlugin : Plugin<Project> {
    ...
    private fun registerPodspecTask(
        ...
        project.tasks.register(POD_SPEC_TASK_NAME, PodspecTask::class.java) {
            ...
            it.publishing = project.provider { false }
            it.pods.set(cocoapodsExtension.pods)
            it.version = project.provider { cocoapodsExtension.version ?: project.version.toString() }
            it.specName = project.provider { cocoapodsExtension.name }
            it.customSpec = project.provider { cocoapodsExtension.customSpec }
            it.outputDir = project.provider { project.projectDir }
    ...
    private fun registerPodspecPublishTask(
        ...
        with(project) {
            val task = tasks.register("podSpec${buildType.name.toLowerCase().capitalize()}", PodspecTask::class.java) { task ->
                ...
                task.outputDir = xcFrameworkTask.map { it.outputDir.resolve(it.buildType.getName()) }
                task.needPodspec = provider { true }
                task.publishing = provider { true }
                task.pods.set(cocoapodsExtension.pods)
                task.specName = provider { cocoapodsExtension.name }
                task.version = provider { cocoapodsExtension.version ?: version.toString() }
                task.customSpec = provider { cocoapodsExtension.customSpec }
                    ...
                    val fatFrameworkTask = if (fatFrameworkTaskName in tasks.names) {
                        ...
                        tasks.register(fatFrameworkTaskName, FatFrameworkTask::class.java) { fatTask ->
                            ...
                            fatTask.destinationDir =
                                fatTask.destinationDir.resolve(framework.buildType.name.toLowerCase()).resolve(framework.baseName).resolve(appleTarget.targetName)
                            fatTask.onlyIf {

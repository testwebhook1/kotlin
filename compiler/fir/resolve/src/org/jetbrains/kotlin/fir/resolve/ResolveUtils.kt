...
fun <T : FirResolvable> BodyResolveComponents.typeFromCallee(access: T): FirResolvedTypeRef {
    return when (val newCallee = access.calleeReference) {
        ...
        is FirSuperReference -> {
            val labelName = newCallee.labelName
            val implicitReceivers = implicitReceiverStack[labelName]
            val implicitReceiver = if (labelName != null) implicitReceivers.singleOrNull() as? ImplicitDispatchReceiverValue
            ...
            resolvedTypeRef ?: buildErrorTypeRef {
                ...
                diagnostic = if (implicitReceivers.size > 1) {
                    ConeAmbiguousLabelError(labelName!!, implicitReceivers.map { it.boundSymbol })
                } else {
                    ConeUnresolvedNameError(Name.identifier("super"))
                }
            }
        }

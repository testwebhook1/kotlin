...
class FirIdeSessionProviderStorage(val project: Project) {
    private val sessionsCache = ConcurrentHashMap<SourceSetProjectModule, FromModuleViewSessionCache>()
    ...
    fun getSessionProvider(
        rootModule: SourceSetProjectModule,
        ...
        val (sessions, session) = cache.withMappings(project) { mappings ->
            val sessions = mutableMapOf<SourceSetProjectModule, FirIdeSourcesSession>().apply { putAll(mappings) }
...
private class FromModuleViewSessionCache(
    val root: SourceSetProjectModule,
) {
    @Volatile
    private var mappings: PersistentMap<SourceSetProjectModule, FirSessionWithModificationTracker> = persistentMapOf()
    ...
    inline fun <R> withMappings(
        project: Project,
        action: (Map<SourceSetProjectModule, FirIdeSourcesSession>) -> Pair<Map<SourceSetProjectModule, FirIdeSourcesSession>, R>
    ): Pair<Map<SourceSetProjectModule, FirIdeSourcesSession>, R> {
    ...
    private fun getSessions(): Map<SourceSetProjectModule, FirIdeSourcesSession> = buildMap {
        ...
        val reversedDependencies = sessions.reversedDependencies { session ->
            session.firSession.module.regularDependencies.mapNotNull { mappings[it] }
        }
...
internal val FirModuleData.module: SourceSetProjectModule get() = moduleUnsafe()

internal inline fun <reified T : ProjectModule> FirModuleData.moduleUnsafe(): T = (this as ModuleProjectBasedModuleData).module as T
internal inline fun <reified T : ProjectModule> FirModuleData.moduleInfoSafe(): T? = (this as ModuleProjectBasedModuleData).module as? T
